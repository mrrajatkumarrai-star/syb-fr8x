export class Sidebar {
  ...
  render() {
    const localData = localStorage.getItem("freightos_erp_db");
    let companyName = "FreightOS";
    let companyLogo = "";
    if (localData) {
      try {
        const dbObj = JSON.parse(localData);
        if (dbObj?.settings?.company) companyName = dbObj.settings.company;
        if (dbObj?.settings?.companyLogo) companyLogo = dbObj.settings.companyLogo;
      } catch(e) {}
    }
    ...
    sb.innerHTML = `
      <div class="sb-logo" style="display:flex; justify-content:space-between; align-items:center; width:100%;">
        <div style="display:flex; align-items:center; gap:12px; cursor:pointer;" id="sb-logo-click">
          <div class="sb-icon">
            ${companyLogo ? `<img src="${companyLogo}" style="width:24px;height:24px;object-fit:contain;border-radius:4px" onerror="this.outerHTML='<i class=\'ti ti-ship\'></i>'">` : `<i class="ti ti-ship" aria-hidden="true"></i>`}
          </div>
          <div>
            <div style="font-size:13px;font-weight:600;color:var(--text-main)">${companyName}</div>
