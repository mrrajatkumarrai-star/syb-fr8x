...
export class Reports {
  ...
  async generateReport(name, format) {
    ...
    if (name === "Shipment Report") {
      ...
      header = "Invoice No,Job No,Customer,Invoice Date,Due Date,Subtotal,GST Type,CGST,SGST,IGST,Total GST,Total,Currency,Status,IRN Status";
      rows = invoices.map(i => `"${i.invNo}","${i.jobNo || ''}","${i.customer}","${i.invoiceDate}","${i.dueDate}",${i.subtotal || 0},"${i.gstType || ''}",${i.cgstTotal || 0},${i.sgstTotal || 0},${i.igstTotal || 0},${i.totalGST || i.gstAmount || 0},${i.total || 0},"${i.currency || 'INR'}","${i.status}","${i.irnStatus || 'draft'}"`);
