...
export class Compass {
          ...
          <tbody>
            ...
            customers.map(c => `
              <tr>
                ...
                <td>${statusBadge(c.isLocked ? 'Locked' : (c.kycStatus || 'Pending'))}</td>
                <td>
                  <div style="display:flex; gap:4px;">
                    ${c.isLocked && currentUserRole === 'Super Admin' ? `<button class="btn sm success" data-role="unlock-customer" data-cid="${c.id}"><i class="ti ti-lock-open"></i> Unlock</button>` : ''}
                    ${c.kycStatus !== 'Approved' && !c.isLocked ? `<button class="btn sm pr" data-role="approve-kyc" data-cid="${c.id}">Approve</button>` : ''}
                    ${c.kycStatus !== 'Rejected' && !c.isLocked ? `<button class="btn sm danger" data-role="reject-kyc" data-cid="${c.id}">Reject</button>` : ''}
                  </div>
                </td>
              </tr>
            `).join('')}
  ...
  setupHandlers(container, allCustomers, allRequests, erpUsers) {
    ...
    if (this.activeTab === "kyc") {
      ...
      // Unlock customer user account
      container.querySelectorAll("[data-role=unlock-customer]").forEach(btn => {
        btn.onclick = () => {
          const cid = btn.dataset.cid;
          const cust = allCustomers.find(c => c.id === cid);
          if (currentUserRole !== "Super Admin") {
            showToast("Only Super Admins can unlock customer accounts.", "error");
            return;
          }
          showConfirm(`Unlock customer portal access for <strong>${cust.companyName}</strong>?`, async () => {
            cust.isLocked = false;
            cust.loginAttempts = 0;
            cust.kycStatus = "Approved"; // Reset status to Approved to allow login
            ...
            await saveDocument("auditLog", {
              ...
              record: `Unlocked customer portal account for ${cust.companyName}`,
              ip: "10.0.0." + Math.floor(Math.random() * 200 + 1)
            });

            showToast(`Unlocked customer account for "${cust.companyName}"`, "success");
            this.refreshPanel(container);
          });
        };
      });
