...
export function makeSearchableSelect(selectEl) {
  ...
  // Check if it already has a searchable wrapper sibling
  const hasContainer = selectEl.previousSibling && 
                       selectEl.previousSibling.nodeType === Node.ELEMENT_NODE && 
                       selectEl.previousSibling.classList.contains('searchable-select-container');
  if (hasContainer) return;

  selectEl.dataset.searchable = "true";

  // Save original style display and check if it's inline
  const computedStyle = window.getComputedStyle(selectEl);
  const isInline = computedStyle.display.startsWith('inline') || selectEl.style.display.startsWith('inline');
  const originalWidth = selectEl.style.width || (selectEl.style.width === '' ? computedStyle.width : '');
  const originalMargin = selectEl.style.margin || computedStyle.margin;
  ...
  container.style.display = isInline ? 'inline-block' : 'block';
  ...
  searchInput.className = 'search-select-input ' + selectEl.className;
  searchInput.placeholder = selectEl.options[0]?.text || 'Select option...';
  
  // Copy inline styles from the select to the input, but override display/width/margin
  searchInput.style.cssText = selectEl.style.cssText;
  searchInput.style.display = 'block';
  searchInput.style.width = '100%';
  searchInput.style.margin = '0';
  searchInput.style.paddingRight = '30px'; // Space for chevron
  
  // Default fallback styles if not styled inline
  if (!selectEl.style.border) searchInput.style.border = '1px solid var(--border)';
  if (!selectEl.style.borderRadius) searchInput.style.borderRadius = 'var(--radius-md)';
  ...
  if (!selectEl.style.fontSize) searchInput.style.fontSize = '11.5px';
  if (!selectEl.style.background) searchInput.style.background = 'var(--bg-card)';
  if (!selectEl.style.color) searchInput.style.color = 'var(--text-main)';
  ...
  chevron.style.cssText = 'position:absolute; right:10px; top:50%; transform:translateY(-50%); color:var(--text-muted); font-size:12px; pointer-events:none; z-index:2;';
  ...
  function updateList(q = '') {
    ...
    opts.forEach((opt, idx) => {
      ...
      item.style.cssText = 'padding:6px 10px; font-size:11px; cursor:pointer; color:var(--text-main); transition:background 0.1s; text-transform:uppercase;';
    ...
    if (count === 0) {
      dropdown.innerHTML = '<div style="padding:6px 10px; font-size:11px; color:var(--text-muted);">No results match</div>';
    }
  }
  ...
  // Synchronize on change event
  const changeListener = () => {
    const selected = selectEl.options[selectEl.selectedIndex];
    searchInput.value = (selected && selected.value !== "") ? selected.text : '';
  };
  selectEl.addEventListener('change', changeListener);

  // Sync if options change inside selectEl
  ...
  // Sync display toggles of the select element itself
  ...
  displayObserver.observe(selectEl, { attributes: true, attributeFilter: ['style', 'class'] });

  // Intercept programmatic changes to selectEl.value and selectEl.selectedIndex
  const descValue = Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype, 'value');
  const descIndex = Object.getOwnPropertyDescriptor(HTMLSelectElement.prototype, 'selectedIndex');

  Object.defineProperty(selectEl, 'value', {
    configurable: true,
    enumerable: true,
    get() {
      return descValue.get.call(this);
    },
    set(val) {
      descValue.set.call(this, val);
  ...
  Object.defineProperty(selectEl, 'selectedIndex', {
    ...
    enumerable: true,
    get() {
      return descIndex.get.call(this);
    },
    set(val) {
      descIndex.set.call(this, val);
      const selected = this.options[this.selectedIndex];
      searchInput.value = (selected && selected.value !== "") ? selected.text : '';
    }
  });
...
export function statusBadge(s) {
  const map = {
    ...
    'Active': 'b-green', 'Inactive': 'b-gray', 'On Hold': 'b-red', 'Locked': 'b-red',
...
export function initGlobalSearchableSelects() {
  // Process existing selects
  document.querySelectorAll('select').forEach(makeSearchableSelect);

  // Process dynamically added selects
  const observer = new MutationObserver((mutations) => {
    for (const mutation of mutations) {
      for (const node of mutation.addedNodes) {
        if (node.nodeType === Node.ELEMENT_NODE) {
          if (node.tagName === 'SELECT') {
            makeSearchableSelect(node);
          } else {
            node.querySelectorAll('select').forEach(makeSearchableSelect);
          }
        }
      }
    }
...
// Automatically initialize global searchable selects for the modular SPA
if (typeof document !== 'undefined') {
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initGlobalSearchableSelects);
  } else {
    initGlobalSearchableSelects();
  }
}
